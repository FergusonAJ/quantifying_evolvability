...
df_10 = read.csv('../../2025_05_04_15__pop_10/data/pairwise_data.csv')
df_10$mut_prob = 0.001
df_10$pop_size = 10
df_10$condition_num = 1

df_100 = read.csv('../../2025_05_04_16__pop_100/data/pairwise_data.csv')
df_100$mut_prob = 0.001
df_100$pop_size = 100
df_100$condition_num = 2

df_10k = read.csv('../../2025_05_04_17__pop_10000/data/pairwise_data.csv')
...
ggplot(df_combined) + 
  ...
  geom_segment(data=df_combined[df_combined$is_signif,], aes(x = x_min, xend = x_max, y = 0.9 - condition_num * 0.06, yend = 0.9 - condition_num * 0.06, color = pop_size_factor), 
              arrow=arrow(type = 'closed', length = unit(0.25, 'cm')), show.legend = F, size = 1.1) + 
  geom_text(data = df_combined[df_combined$is_signif,], aes(x = text_x - 0.05, y = 0.86 - condition_num * 0.06, label = signif_marker, color = pop_size_factor), show.legend = F, size = 6) + 
  ...
  scale_y_continuous(limits = c(0.62, 1.8), breaks = c(1, 1.25, 1.5, 1.75), minor_breaks = c(1, 1.125, 1.25, 1.375, 1.5, 1.625, 1.75)) + 
