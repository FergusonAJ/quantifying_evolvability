...
df_mut_low = read.csv('../../2025_05_04_21__mut_0_0005/data/pairwise_data.csv')
df_mut_low$mut_prob = 0.0005
df_mut_low$pop_size = 1000
df_mut_low$condition_num = 1

df_mut_very_low = read.csv('../../2025_05_04_22__mut_0_0001/data/pairwise_data.csv')
df_mut_very_low$mut_prob = 0.0001
df_mut_very_low$pop_size = 1000
df_mut_very_low$condition_num = 2

df_mut_high = read.csv('../../2025_05_04_23__mut_0_005/data/pairwise_data.csv')
df_mut_high$mut_prob = 0.005
df_mut_high$pop_size = 1000
df_mut_high$condition_num = 4

df_mut_very_high = read.csv('../../2025_05_04_24__mut_0_01/data/pairwise_data.csv')
...
output_filename = paste0(data_dir, '/mut_rate_combined_data.csv')
...
ggplot(df_combined) + 
  geom_segment(data=df_combined[df_combined$pop_size == 1000 & df_combined$mut_prob == 0.001,], aes(x = genotype_a_x, xend = genotype_b_x, y = starting_fitness_a_norm, yend = starting_fitness_b_norm, color = 'Starting fitness'), 
               size = 1.05) + 
  ...
  geom_segment(data=df_combined[df_combined$is_signif,], aes(x = x_min, xend = x_max, y = y, yend = y, color = mut_factor), 
              arrow=arrow(type = 'closed', length = unit(0.25, 'cm')), size = 1.15, show.legend = F) + 
  geom_text(data = df_combined[df_combined$is_signif,], aes(x = text_x, y = text_y, label = signif_marker, color = mut_factor), size = 6, show.legend = F) + 
  xlab('Genotype') + 
  ylab('Fitness') + 
  labs(color = '') +
  scale_color_manual(values = color_map) + 
  scale_y_continuous(limits = c(0.95, 1.8), breaks = c(1, 1.25, 1.5, 1.75), minor_breaks = c(1, 1.125, 1.25, 1.375, 1.5, 1.625, 1.75)) + 
