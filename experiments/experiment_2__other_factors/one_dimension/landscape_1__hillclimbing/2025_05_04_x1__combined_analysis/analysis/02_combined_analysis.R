...
ggp_mut = ggplot(df_combined[mut_mask,]) + 
  geom_segment(data=df_combined[mut_mask & baseline_mask & order_mask,], aes(x = genotype_a_x, xend = genotype_b_x, y = starting_fitness_a_norm, yend = starting_fitness_b_norm, color = 'Starting fitness'), linetype = 'dashed', size = 1.05) + 
...
print(ggp_mut)
ggsave(paste0(plot_dir, '/mut_only.pdf'), units = 'in', width = 6, height = 5)
ggsave(paste0(plot_dir, '/mut_only.png'), units = 'in', width = 6, height = 5)

print(ggp_pop)
ggsave(paste0(plot_dir, '/pop_only.pdf'), units = 'in', width = 6, height = 5)
ggsave(paste0(plot_dir, '/pop_only.png'), units = 'in', width = 6, height = 5)
