...
ggp_mut = ggplot(df_combined[mut_mask,]) + 
  ...
  geom_text(data = df_combined[mut_mask & df_combined$is_signif,], aes(x = text_x, y = text_y - 0.0025, label = signif_marker, color = mut_factor), size = 6, show.legend = F) + 
  xlab('Genotype') + 
  ylab('Fitness') + 
  labs(color = '') +
...
ggp_pop = ggplot(df_combined[pop_mask,]) + 
  ...
  geom_segment(data=df_combined[pop_mask & df_combined$is_signif,], aes(x = x_min, xend = x_max, y = y, yend = y, color = pop_size_factor), 
              arrow=arrow(type = 'closed', length = unit(0.15, 'cm')), size = 1.15, show.legend = F) + 
  geom_text(data = df_combined[pop_mask & df_combined$is_signif,], aes(x = text_x, y = text_y-0.0025, label = signif_marker, color = pop_size_factor), size = 6, show.legend = F) + 
  xlab('Genotype') + 
  ylab('Fitness') + 
  labs(color = '') +
...
ggsave(paste0(plot_dir, '/combined_summary_plot.pdf'), units = 'in', width = 12, height = 4)
ggsave(paste0(plot_dir, '/combined_summary_plot.png'), units = 'in', width = 12, height = 4)