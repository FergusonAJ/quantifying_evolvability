...
df_pairwise = data.frame(data = matrix(nrow = 0, ncol = 8))
colnames(df_pairwise) = c('genotype_a', 'genotype_b', 'avg_fitness_a', 'avg_fitness_b', 'raw_p', 'adj_p', 'starting_fitness_a', 'starting_fitness_b')
for(genotype in unique(df_evo$starting_index)){
  ...
  for(bit_idx in 1:num_bits){
    ...
    if(sum(genotype_max_fitness) == sum(neighbor_max_fitness)){
      df_pairwise[nrow(df_pairwise) + 1,] = c(genotype, neighbor_genotype, mean(genotype_max_fitness), mean(neighbor_max_fitness), 1, NA, starting_fitness_a, starting_fitness_b)
    } else{
      df_pairwise[nrow(df_pairwise) + 1,] = c(genotype, neighbor_genotype, mean(genotype_max_fitness), mean(neighbor_max_fitness), res$p.value, NA, starting_fitness_a, starting_fitness_b)
    }
  }
